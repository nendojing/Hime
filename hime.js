var BLOCKSIZE = 32;   // 磚塊大小
var BALLSIZE  = 30	;   // 球大小
var ANGLE_MIN = 10;   // 
var PDL_EDGE  = 0.2;  // 板子角度
...
function putImage(margin, imgs)  // 放入圖片
{
	var i, wk, str = "", w = h = 0;

	imgs = [].concat.apply([], imgs);  
...
var block = new function() {
	this.create = function(img) {
		 ...
		 *
		 */
		var y1 =  0 * w * 4,  //(X,0) (X,5) (X,10) (X,15)
			y2 =  5 * w * 4,
			y3 = 10 * w * 4,
			y4 = 15 * w * 4;

		var x1 =  0 * 4 + 3,  //(0,Y) (5,Y) (10,Y) (15,Y)
			x2 =  5 * 4 + 3,
			x3 = 10 * 4 + 3,
			x4 = 15 * 4 + 3;
...
var scene = new function() {
	...
	function ending() {   
		...
		if (conf.next) {
			puts("Next Page", "under");
			clickDo(function(){ location.href = conf.next; });
		}
		else {
			puts("Congratulation!", "under");
		}
	}
		//標題畫面
	this.title = function() {
...
window.onload = function()
{
	...
	msg("THE iDOLM@STER BREAKOUT GAME v0.01\n\n");
	msg("Now Loading... ");
